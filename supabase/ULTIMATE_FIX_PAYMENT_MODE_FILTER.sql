-- ============================================================================
-- ULTIMATE FIX: Payment Mode Filter Logic for Ledger Statement
-- Date: 2025-11-03
-- Purpose: Fix WHERE clause to compute payment_mode same as SELECT clause
--
-- ROOT CAUSE IDENTIFIED:
-- - WHERE clause checks ap.payment_mode which can be NULL
-- - But SELECT clause uses COALESCE + CASE to infer payment_mode
-- - WHERE clause must use SAME logic as SELECT clause
-- ============================================================================
...
BEGIN
  ...
  SELECT DISTINCT ON (ve.id)
    ...
    COALESCE(ap.visit_id::TEXT, vis.visit_id::TEXT, v.reference_number::TEXT, '') as visit_id,
    ...
    -- Payment Mode - Computed with COALESCE + CASE
    COALESCE(
  ...
  WHERE
    ...
    -- CRITICAL FIX: Compute payment_mode using SAME logic as SELECT clause
    AND (
      ...
      -- Filter specified - compute payment_mode and compare
      (
        COALESCE(
          ap.payment_mode::TEXT,
          CASE
...
COMMENT ON FUNCTION get_ledger_statement_with_patients IS 'Ultimate fix: WHERE clause now computes payment_mode identically to SELECT clause using COALESCE + CASE';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
  RAISE NOTICE '✅ ULTIMATE FIX APPLIED - Payment Mode Filter CORRECTED!';
  RAISE NOTICE '';
  RAISE NOTICE 'Key Fix:';
  RAISE NOTICE '  - WHERE clause now uses COALESCE + CASE (same as SELECT)';
  RAISE NOTICE '  - Computes payment_mode BEFORE comparing to filter';
  RAISE NOTICE '  - Bank accounts will be correctly inferred as ONLINE';
  RAISE NOTICE '  - NULL payment_mode values handled properly';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '  5. Payment Mode: ONLINE';
  RAISE NOTICE '  6. Your 2 x ₹10 payments WILL NOW appear!';
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
END $$;
