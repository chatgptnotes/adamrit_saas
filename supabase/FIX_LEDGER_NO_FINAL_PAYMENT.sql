-- ============================================================================
-- Fix Ledger Statement JOIN Condition (Corrected - No final_payment)
-- Date: 2025-11-03
...
-- NOTE: Removed final_payment references as table doesn't exist
-- ============================================================================
...
BEGIN
  ...
  SELECT DISTINCT ON (ve.id)  -- ONE ROW PER VOUCHER ENTRY!
    ...
    -- Payment Type - Try advance_payment and voucher type
    CASE
  ...
  -- IMPROVED Payment JOIN - More flexible matching
...
-- Add helpful comment
COMMENT ON FUNCTION get_ledger_statement_with_patients IS 'Fixed version: Shows ALL voucher entries for account, with improved JOIN logic. Infers payment_mode for bank accounts. No dependency on final_payment table.';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
  RAISE NOTICE '✅ Ledger function updated successfully! (Corrected version)';
  RAISE NOTICE '';
  RAISE NOTICE 'Changes made:';
  RAISE NOTICE '  1. Removed final_payment table references';
  RAISE NOTICE '  2. More flexible JOIN conditions for advance_payment';
  RAISE NOTICE '  3. Payment mode inference for bank transactions';
  RAISE NOTICE '  4. Shows ALL voucher entries even if JOIN fails';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '  4. Date: 01-11-2025 to 03-11-2025';
  RAISE NOTICE '  5. Your 2 x ₹10 payments should now appear!';
  RAISE NOTICE '═══════════════════════════════════════════════════════════';
END $$;
